...
<body>
    ...
    <div class="content">
        <h2>Everton FC Details</h2>
        <div class="team-container">
            <?php
            // File koneksi.php untuk koneksi ke database
            include 'koneksi.php';

            // API Endpoint untuk Everton FC
            $url = 'https://api.football-data.org/v2/teams/62'; // ID 62 adalah Everton FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Everton FC
            function getEvertonData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Everton FC
            $everton_data = getEvertonData($url, $api_key);

            // Tampilkan data Everton FC
            if ($everton_data) {
                echo "<img src='{$everton_data['crestUrl']}' alt='{$everton_data['name']}'>";
                echo "<h3>{$everton_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$everton_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$everton_data['clubColors']}</p>";
                if (isset($everton_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$everton_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Data not available</p>";
                }
                echo "<p><strong>Email:</strong> {$everton_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$everton_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($everton_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($everton_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Everton beserta posisi dari database
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Everton'";
