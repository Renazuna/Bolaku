...
<head>
    ...
    <title>BOLAKU - Brentford FC</title>
...
<body>
    ...
    <div class="content">
        <h2>Brentford FC Details</h2>
        <div class="team-container">
            <?php
            // API Endpoint untuk Brentford FC
            $url = 'https://api.football-data.org/v2/teams/402'; // ID 402 adalah Brentford FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Brentford FC
            function getBrentfordData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Brentford FC
            $brentford_data = getBrentfordData($url, $api_key);

            // Tampilkan data Brentford FC
            if ($brentford_data) {
                echo "<img src='{$brentford_data['crestUrl']}' alt='{$brentford_data['name']}'>";
                echo "<h3>{$brentford_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$brentford_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$brentford_data['clubColors']}</p>";
                if (isset($brentford_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$brentford_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Data not available</p>";
                }
                echo "<p><strong>Email:</strong> {$brentford_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$brentford_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($brentford_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($brentford_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Brentford FC beserta posisi
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Brentford'";
