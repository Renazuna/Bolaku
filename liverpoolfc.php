...
<body>
    ...
    <div class="content">
        <h2>Liverpool FC Details</h2>
        <div class="team-container">
            <?php
            // API Endpoint untuk Liverpool FC
            $url = 'https://api.football-data.org/v2/teams/64'; // ID 64 adalah Liverpool FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Liverpool FC
            function getLiverpoolData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Liverpool FC
            $liverpool_data = getLiverpoolData($url, $api_key);

            // Tampilkan data Liverpool FC
            if ($liverpool_data) {
                echo "<img src='{$liverpool_data['crestUrl']}' alt='{$liverpool_data['name']}'>";
                echo "<h3>{$liverpool_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$liverpool_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$liverpool_data['clubColors']}</p>";
                if (isset($liverpool_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$liverpool_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Arne Slot</p>";
                }
                echo "<p><strong>Email:</strong> {$liverpool_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$liverpool_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($liverpool_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($liverpool_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Liverpool beserta posisi
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Liverpool'";
