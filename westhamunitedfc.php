...
<head>
    ...
    <title>BOLAKU - West Ham United FC</title>
...
<body>
    ...
    <div class="content">
        <h2>West Ham United FC Details</h2>
        <div class="team-container">
            <?php
            // API Endpoint untuk West Ham United FC
            $url = 'https://api.football-data.org/v2/teams/563'; // ID 563 adalah West Ham United FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data West Ham United FC
            function getWestHamData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data West Ham United FC
            $westham_data = getWestHamData($url, $api_key);

            // Tampilkan data West Ham United FC
            if ($westham_data) {
                echo "<img src='{$westham_data['crestUrl']}' alt='{$westham_data['name']}'>";
                echo "<h3>{$westham_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$westham_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$westham_data['clubColors']}</p>";
                if (isset($westham_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$westham_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Data not available</p>";
                }
                echo "<p><strong>Email:</strong> {$westham_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$westham_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($westham_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($westham_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain West Ham United FC beserta posisi
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'West Ham United'";
