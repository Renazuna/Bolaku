...
<head>
    ...
    <title>BOLAKU - Manchester City</title>
...
<body>
    ...
    <div class="content">
        <h2>Manchester City FC Details</h2>
        <div class="team-container">
            <?php
            // API Endpoint untuk Manchester City FC
            $url = 'https://api.football-data.org/v2/teams/65'; // ID 65 adalah Manchester City FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Manchester City FC
            function getManCityData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Manchester City FC
            $man_city_data = getManCityData($url, $api_key);

            // Tampilkan data Manchester City FC
            if ($man_city_data) {
                echo "<img src='{$man_city_data['crestUrl']}' alt='{$man_city_data['name']}'>";
                echo "<h3>{$man_city_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$man_city_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$man_city_data['clubColors']}</p>";
                if (isset($man_city_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$man_city_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Pep Guardiola</p>";
                }
                echo "<p><strong>Email:</strong> {$man_city_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$man_city_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($man_city_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($man_city_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Manchester City beserta posisi
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Man City'";
