...
<body>
    ...
    <div class="content">
        <h2>Chelsea FC Details</h2>
        <div class="team-container">
            <?php
            // File koneksi.php untuk koneksi ke database
            include 'koneksi.php';

            // API Endpoint untuk Chelsea FC
            $url = 'https://api.football-data.org/v2/teams/61'; // ID 61 adalah Chelsea FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Chelsea FC
            function getChelseaData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Chelsea FC
            $chelsea_data = getChelseaData($url, $api_key);

            // Tampilkan data Chelsea FC
            if ($chelsea_data) {
                echo "<img src='{$chelsea_data['crestUrl']}' alt='{$chelsea_data['name']}'>";
                echo "<h3>{$chelsea_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$chelsea_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$chelsea_data['clubColors']}</p>";
                if (isset($chelsea_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$chelsea_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Enzo Maresca</p>";
                }
                echo "<p><strong>Email:</strong> {$chelsea_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$chelsea_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($chelsea_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($chelsea_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Chelsea beserta posisi dari database
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Chelsea'";
