...
<body>
    ...
    <div class="content">
        <h2>Arsenal Details</h2>
        <div class="team-container">
            <?php
            // File koneksi.php untuk koneksi ke database
            include 'koneksi.php';

            // API Endpoint untuk Arsenal
            $url = 'https://api.football-data.org/v2/teams/57'; // ID 57 adalah Arsenal

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Arsenal
            function getArsenalData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Arsenal
            $arsenal_data = getArsenalData($url, $api_key);

            // Tampilkan data Arsenal
            if ($arsenal_data) {
                echo "<img src='{$arsenal_data['crestUrl']}' alt='{$arsenal_data['name']}'>";
                echo "<h3>{$arsenal_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$arsenal_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$arsenal_data['clubColors']}</p>";
                if (isset($arsenal_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$arsenal_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Mikel Arteta</p>";
                }
                echo "<p><strong>Email:</strong> {$arsenal_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$arsenal_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($arsenal_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($arsenal_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Arsenal beserta posisi dari database
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Arsenal'";
