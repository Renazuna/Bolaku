...
<body>
    ...
    <div class="content">
        <h2>Fulham FC Details</h2>
        <div class="team-container">
            <?php
            // API Endpoint untuk Fulham FC
            $url = 'https://api.football-data.org/v2/teams/63'; // ID 63 adalah Fulham FC

            // API key Anda dari Football-Data.org
            $api_key = 'bc8aeeaf9bcc43f8a5feedab796264a7';

            // Fungsi untuk mendapatkan data Fulham FC
            function getFulhamData($url, $api_key) {
                $options = [
                    'http' => [
            ...
            // Ambil data Fulham FC
            $fulham_data = getFulhamData($url, $api_key);

            // Tampilkan data Fulham FC
            if ($fulham_data) {
                echo "<img src='{$fulham_data['crestUrl']}' alt='{$fulham_data['name']}'>";
                echo "<h3>{$fulham_data['name']}</h3>";
                echo "<p><strong>Stadium:</strong> {$fulham_data['venue']}</p>";
                echo "<p><strong>Club Colors:</strong> {$fulham_data['clubColors']}</p>";
                if (isset($fulham_data['squad']['coach']['name'])) {
                    echo "<p><strong>Manager:</strong> {$fulham_data['squad']['coach']['name']}</p>";
                } else {
                    echo "<p><strong>Manager:</strong> Marco Silva</p>";
                }
                echo "<p><strong>Email:</strong> {$fulham_data['email']}</p>";
                echo "<p><strong>Phone:</strong> {$fulham_data['phone']}</p>";
                echo "<div class='competitions'>";
                echo "<p><strong>Active Competitions:</strong></p>";
                if (isset($fulham_data['activeCompetitions'])) {
                    echo "<ul>";
                    foreach ($fulham_data['activeCompetitions'] as $competition) {
            ...
            // Query untuk mengambil nama-nama pemain Fulham beserta posisi
            $query = "SELECT Player, Position FROM player_stats WHERE Club = 'Fulham'";
